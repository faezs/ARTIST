...
CONFIG = {
    ...
    "fm_machine": dict(title="FACT mount, equinox noon: fork on the deck ring, blade trunnions on the axis through F, cradle, fine stage",
                       iso=(-1.0, -1.0, 0.55), bounds_exclude=("sun",), context=("deck", "wall", "column", "pot", "rail"),
                       detail=(200, 2000, -3900, -2100, 6900, 10300), detail_label="detail: the west trunnion block on its post, the arm clamp, the crank and the jack"),
    "fm_summer": dict(title="Summer noon, el 83: the dish flat under F, the tube through the slot, the arms upright beside it", iso=(-1.0, -1.0, 0.55),
                      bounds_exclude=("sun",), context=("deck", "wall", "column", "pot", "rail"), detail=None),
    "fm_morning": dict(title="Equinox 9 h, el 38 ESE: the fork turned, the cradle tilted, nothing in front of the dish", iso=(-1.0, -1.0, 0.55),
                       bounds_exclude=("sun",), context=("deck", "wall", "column", "pot", "rail"), detail=None),
    "fm_sweep": dict(title="Equinox day, 8-16 h: rims, arms, posts and the elevation axis, every axis through F", iso=(-1.0, -1.0, 0.7),
                     bounds_exclude=("sun",), context=("deck", "column", "pot", "rail"), detail=None),
    "fm_pivot": dict(title="Trunnion block: twelve blades in two cross-blade stages on the elevation axis, 1 DOF Type 1; post below, arm inboard",
                     iso=(1.0, -1.2, 0.7), bounds_exclude=("axis",), context=("stub", "(top)"),
                     detail=(-130, 130, -690, -30, -130, 130), detail_label="detail: stage B, three cells of two blades crossing on the axis, over the moving bar"),
    "fm_fine": dict(title="Fine stage: tangential blades in the back plane, struts on the normals, water columns; C-rings open at the slot",
                    iso=(1.0, 1.0, -0.8), bounds_exclude=("axis",), context=("plenum",),
                    detail=(100, 1000, 1000, 1800, -700, -150), detail_label="detail: one blade to its post, one strut with its necks on its column"),
    "tw_equinox_noon": dict(title="The flower as simulated, equinox noon: a 1 m steel stem on the deck, the pedicel's telescoping boom lifting the receptacle ring, six 1.45 m struts on flexure balls to the platform ring, the membrane on its calyx; the light pipe separate",
                            iso=(-1.0, -1.0, 0.55), bounds_exclude=("sun",), context=("deck", "light pipe"),
                            detail=(1600, 4800, -1600, 1600, 5000, 7800), detail_label="detail: the boom's wrist, the receptacle ring, the six struts, the platform ring and the calyx under the membrane"),
    "tw_winter_noon": dict(title="Winter (day 35) noon on the env's law: the head on the orbit sphere, the boom out to it, the struts at their constant 1.45 m", iso=(-1.0, -1.0, 0.55), bounds_exclude=("sun",), context=("deck", "light pipe")),
    "tw_sweep": dict(title="Where the pedicel carries the head on the env's law (orbit sphere, axis the bisector, beta within 36 deg, the rim 0.3 m clear of the pipe): 9, 12, 15 h equinox, winter and summer noon", iso=(-1.0, -1.0, 0.55), bounds_exclude=("sun",), context=("deck", "light pipe")),
    "tr_equinox_noon": dict(title="The flower proper, equinox noon: one stem on the deck, a crown of branches carrying the exact spherical membrane, F at the mouth of the roof's light pipe",
                            iso=(-1.0, -1.0, 0.55), bounds_exclude=("sun",), context=("deck", "light pipe"),
                            detail=(1000, 3200, -1400, 1400, 5000, 7200), detail_label="detail: the stem, the receptacle, the primaries, secondaries and twigs under the membrane"),
    "tr_winter_noon": dict(title="Winter noon, el 36: the head 3.2 m north and 2.5 m up on the orbit about F, the branches reaching", iso=(-1.0, -1.0, 0.55), bounds_exclude=("sun",), context=("deck", "light pipe")),
    "tr_equinox_morning": dict(title="Equinox 9 h, el 37: the head 2.8 m west of the pipe's meridian; the crown turns about the stem", iso=(-1.0, -1.0, 0.55), bounds_exclude=("sun",), context=("deck", "light pipe")),
    "tr_summer_noon": dict(title="Summer noon, el 83: the head low beside the pipe, looking up, beta taking it clear of the pipe", iso=(-1.0, -1.0, 0.55), bounds_exclude=("sun",), context=("deck", "light pipe")),
    "tr_sweep": dict(title="The year's sweep: the head on the orbit sphere at 8, 12 and 16 h equinox and at the solstice noons; the branches follow", iso=(-1.0, -1.0, 0.55), bounds_exclude=("sun",), context=("deck", "light pipe")),
    "cd_equinox_noon": dict(title="Everything behind the dish, equinox noon: Cassegrain head on its style, hollow flexure neck, one stalk, the beam down inside",
                            iso=(-1.0, -1.0, 0.55), bounds_exclude=("sun",), context=("deck", "wall", "bore", "pot", "underground"),
                            detail=(-200, 3200, -3000, 700, 5800, 8200), detail_label="detail: M3 at the neck, the hollow cartwheel pivot, M4 in the yoke at the stalk top"),
    "cd_summer_noon": dict(title="Summer noon, el 83: the dish flat over the neck, nothing above it but the style and the secondary", iso=(-1.0, -1.0, 0.55),
                           bounds_exclude=("sun",), context=("deck", "wall", "bore", "pot", "underground"), detail=None),
    "cd_morning": dict(title="Equinox 9 h, el 38 ESE: the stalk, yoke, pivot, counterweights and jack all behind the membrane", iso=(-1.0, -1.0, 0.55),
                       bounds_exclude=("sun",), context=("deck", "wall", "bore", "pot", "underground"), detail=None),
    "cd_sweep": dict(title="Equinox day, 8-16 h: rims, neck bars, spines and secondary axes about one stalk", iso=(-1.0, -1.0, 0.7),
                     bounds_exclude=("sun",), context=("deck", "bore", "pot", "underground"), detail=None),
    "cd_pivot": dict(title="Hollow cartwheel trunnion: two stages of six radial blades in planes through the neck axis around the beam's bore",
                     iso=(1.0, -1.2, 0.7), bounds_exclude=("axis", "beam"), context=("stub",),
                     detail=(-520, 520, -350, 50, -520, 520), detail_label="detail: stage A, six blades from the ground ring to the intermediate ring, the bore open"),

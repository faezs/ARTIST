...
async function mountAnim(el, anim) {
  ...
  function setFrame(fr) {
    ...
    if (lg) { tgtM.position.set(lg.tgt[0], lg.tgt[1], lg.tgt[2]); const d = new THREE.Vector3(Fs.position.x - lg.tgt[0], Fs.position.y - lg.tgt[1], Fs.position.z - lg.tgt[2]).normalize(); tgtM.lookAt(tgtM.position.clone().add(d));
      ...
      const T = anim.t_setup, stalk = anim.sec !== undefined, ph = lg.t < 0.08 * T ? 'stowed, tubes pressurised' : lg.t < 0.42 * T ? (stalk ? 'the stalk grows to the neck\'s height' : 'posts grow to the height of F') : lg.t < 0.66 * T ? 'counterweight tubes grow' : lg.t < 0.74 * T ? 'water pumped into the counterweights' : lg.t < T ? 'the jack turns the head to the morning sun' : 'tracking the sun';
      label.textContent = `t ${lg.t.toFixed(1)} s · ${ph} · sun ${lg.hour.toFixed(1)} h · ${stalk ? 'neck' : 'axis'} z ${(lg.z_axis + anim.z_offset).toFixed(2)} m · vertex error ${lg.err.toFixed(2)} m · coarse ${(lg.coarse !== undefined ? lg.coarse : lg.point).toFixed(1)}° · dish ${lg.point < 0.1 ? (lg.point * 17.45).toFixed(2) + ' mrad' : lg.point.toFixed(2) + '°'} · struts ${Math.max.apply(null, lg.muscles).toFixed(0)} N`; }
    place();
  }

...
TREE.append(plate(60, "Chapter 6 used: Hopkins's four FACT steps on the flower's twelve strut joints, a flexure ball at each strut end", TP + "fact_ball.png",
  ...
  [("motion", f"the six leg lengths are constant over the day (both rings are defined from the head pose), so each ball turns only with the loop's stroke: +-{_fb['theta_deg']:.1f} deg, and 0.08 deg of dither"),
   ...
   ("load", f"{_fb['P_leg']:.0f} N per leg in the runs' worst case, {_fb['P_leg_survival']:.0f} N at the 40 m/s survival gust"),
   ("blades", f"Ti 6Al-4V {_fb['blade']['t_mm']:.1f} x {_fb['blade']['w_mm']:.0f} mm, {_fb['blade']['L_mm']:.0f} mm free: {_fb['blade']['sigma_MPa']:.0f} MPa at +-1.6 deg, Euler {_fb['blade']['P_cr']/1e3:.0f} kN about the thin axis; the joint {_fb['k_joint_MN_m']:.0f} MN/m axially, twice the strut's own EA/L"),
   ("why not wires", _find(_fbl, "  the wire tripod holds").lstrip())],
  "Twelve joints, 72 blades, no bearings, no backlash, no lubrication: the joints the flower wanted from the start. stage3/topopt/fact_ball.py."))
TREE.append(plate(61, "Chapter 7 used: Frecker's ground-structure method on the book's own examples, then on the flower's calyx and receptacle", TP + "ground_truss.png",
  ...
  [("inverter", f"u_out {_gt['inverter']['u_out']:+.2f} for u_in {_gt['inverter']['u_in']:+.2f}: the diamond of Fig. 7.3a, output against input"),
   ("pliers", f"jaw {_gt['pliers']['u_out']:+.3f} for handle {_gt['pliers']['u_in']:+.2f}: a compliant pivot pair over the symmetry line"),
   ("calyx", f"rim (24 nodes) to the six platform anchors under the 15 m/s peak drag, moment and weight: {_gt['calyx']['n_members']} members, {_gt['calyx']['mass']:.0f} kg Al at the chosen volume fraction, rim tilt {_gt['calyx']['rim_tilt_mrad']:.2f} mrad, stress {_gt['calyx']['max_stress']/1e6:.0f} MPa (stiffness, not strength, sizes it)"),
   ("receptacle", f"six anchors to the wrist under +-3.5 kN alternating: {_gt['receptacle']['n_members']} members, {_gt['receptacle']['mass']:.0f} kg, anchor displacement {_gt['receptacle']['anchor_disp_mm']:.1f} mm, stress {_gt['receptacle']['max_stress']/1e6:.0f} MPa"),
...
TREE.append(plate(62, "The head as built, not as a plate: Mylar on a ring, pumped to f 4, the wind through the film into the ring and the spider, and the film's own figure under gusts with its pressure under control", TP + "membrane_wind.png",
  ...
  [("at work", _find(_mwl, "1. the membrane").split(": ", 1)[1]),
   ...
   ("with control", "no control: the 9 m/s mean alone defocuses 3.3 cm, the gusts 1.7 cm rms (6.0 and 3.0 at 12 m/s); the RL policy's one action per 20 s trims the mean and leaves 85-91 % of the gust power; a local pressure loop at 0.5 Hz leaves 0.5 / 0.9 / 1.6 cm at 9 / 12 / 15 m/s"),
   ("the compliant canceller", _find(_mwl, "   the film's volume").strip() + " " + _find(_mwl, "   a 1.0 m2").strip()),
  ...
  "The gust canceller is a compliant mechanism: a constant-force element (Handbook 12.3.3) on a rolling diaphragm holds the plenum's differential pressure at the RL policy's setpoint while air flows to and from the film's changing volume, at acoustic speed, with no sensor. What it cannot touch is the pitching moment's gradient, an n = 1 mode with zero mean slope: no pointing error, only blur, the membrane's floor under wind. stage3/topopt/membrane_wind.py, ring_calyx.py."))

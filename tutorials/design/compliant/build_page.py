...
def setup_rows(log_path, anim_path):
    ...
    return [("what runs", f"{An['n_particles']} particles, {len(An['tris'])//3} membrane triangles, {An['n_frames']} frames at {1/An['dt']:.0f} fps: one stalk r 0.5 m and two counterweight hoses r 0.35 m as closed fabric tubes; the yoke, the head (frame 70 kg, dish + style + secondary 85 kg) as spring trusses; the hollow neck as a hinge on its axis"),
            ...
            ("turn", f"the jack from the yoke to a 0.8 m crank on the neck bar (15 kN cap) turns the head from el 90 to the morning sun {0.74*T:.0f}-{0.98*T:.0f} s; jack peak {mus/1e3:.1f} kN"),
            ...
            ("controller model", (lambda k: k[0] + "; " + k[1] if len(k) > 1 else k[0])(open("stage3/setup_sim/out/koopman4.txt").read().strip().splitlines()) + " (pykoopman, the approach of Bruder et al. for pneumatic soft arms: the lifted linear model a model-predictive controller runs at the struts' valves and the ring drive)")]
SETUP = []
...
BEHIND.append(plate3d(46, "Everything behind the dish: Cassegrain head on its style, hollow flexure neck, one stalk, the beam down inside (equinox noon)", CD + "cd_equinox_noon.json",
  ...
  [("optics", f"secondary r {_O['r_sec']:.2f} m at {_O['D_SEC']} m (magnification {_O['m']:.1f}, f_eff {_O['f_eff']:.0f} m); hole r {_O['r_hole']:.2f} m; image r {_O['r_img']:.2f} m 1.0 m behind the vertex; M3 r {_O['r_m3']:.2f} m, M4 r {_O['r_m4']:.2f} m, both at 45 deg at every elevation"),
   ...
   ("head", "dish + style + secondary + M3 on the diaphragm fine stage (sheets 38-39) on a short frame; water counterweights 151 kg behind the neck and 267 kg beyond the stalk (the head and its neck counterweight both sit 1.4 m from the stalk), both in the shadow; screw jack 1.40-2.11 m from the yoke to a 0.8 m crank 20 deg from -n toward the dish (arm >= 0.39 m): 4 / 31 kN at 9 / 25 m/s"),

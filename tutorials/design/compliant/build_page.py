...
TREE.append(plate(52, "The flower proper: one stem on the deck, a crown of branches carrying the exact spherical membrane, F at the mouth of the roof's light pipe (equinox noon)", TR + "tr_equinox_noon_views_small.png",
  ...
  [("primary", "the exact hashemi.ini membrane: a spherical cap a 2.1 m, R 8 m, f = g = 4 m, no hole, no slot; the crown's tips carry it"),
   ...
   ("F", "at the mouth of the light pipe, a vertical pipe r 0.7 on the roof to z_deck + 0.35 + max(g sin el + a cos el) = 4.87 m, down through the roof to the oven; the hyperboloid strip r 0.6 at F - 0.6: a Masdar beam-down; the pipe is separate of the flower and the beam never passes through the dish (no hole in the membrane)"),
   ("the pipe's price", "the schedule (stage3/tree/path.py) keeps the whole membrane 0.3 m clear of the pipe; with beta capped at 36 deg that is impossible when the sun is higher than about 75 deg: the hour either side of summer noon, 11 of 479 sun samples, is lost unless beta is allowed further or F is offset from the pipe"),
...
TREE.append(plate(55, "Summer, 11 h, el 75, beta 31: the highest sun the flower can serve beside the pipe", TR + "tr_summer_noon_views_small.png", "summer noon itself (el 83) would put the head under F with the pipe through the membrane; without a hole it is not drawn",
...
TREE.append(plate(56, "The flower as simulated: a 1 m steel stem on the deck, the pedicel's boom lifting the receptacle ring, six 1.45 m struts on flexure balls, the membrane on its calyx (equinox noon on the env's law)", TR + "tw_equinox_noon_views_small.png",
  "The stem carries the pedicel, the pedicel carries the receptacle ring to 1.8 m behind the head, coaxial with it, and the six struts hold the head from the ring: nothing hangs from the pipe, which only receives the light. The rows below are wind_size.py's sizing of the struts, stem and calyx under the audit's loads (peak factor 3 on the mean wind; 40 m/s 3-s gust; lift and hinge moment); wind_size.py placed the receptacle at the stem top, so its leg lengths and forces are that geometry's, an upper bound on the pedicel's 1.45 m struts, which carried 5 kN at most in the simulations. A cantilever crown cannot hold the head to 5 cm at F (its tip rotation alone costs 2 f theta, audit F8); a hexapod can. The pipe is drawn as the env has it: the bore from F through the membrane's 0.5 m hole to the chase, the strip on an arm from a tower 3 m north (arm_north), not a tube under F (audit F3).",
...
TREE.append(plate(57, "Where the pedicel carries the head on the env's own law (orbit sphere, axis the bisector, beta within 36 deg, the membrane clear of the pipe): 9, 12 and 15 h at the equinox, winter noon and summer 11 h", TR + "tw_sweep_views_small.png",
  _pt[0],
  ...
  "Hidden-line projection of the CadQuery model; mm. The poses are the schedule's: the boom 0.6-3.3 m, beta up to 36 deg, the whole membrane 0.3 m clear of the r 0.7 pipe."))
...
def _find(lines, start): return next((l.strip() for l in lines if l.strip().startswith(start.strip())), "")
_cT = _jt.load(open(TP + "fact_ball_tripod_ti_cad.json")); _cN = _jt.load(open(TP + "fact_ball_neck_ti_cad.json"))
_pT = _cT["part"]; _pN = _cN["part"]
TREE.append(plate(60, "Chapter 6 used, then built: Hopkins's four FACT steps on the flower's twelve strut joints, and the joint as a solid you can cut or print", TP + "fact_ball_cad.png",
  "Step 1 the DOFs (three rotations about the joint centre C); step 2 the freedom space (the sphere of rotation lines, 3-DOF Type 3) and its constraint space by screw reciprocity (every line through C); step 3 three independent lines, the wire tripod of Fig. 6.8c; step 4 the equivalence of Fig. 6.7a, each wire as two orthogonal blades in series (Fig. 6.8d). Every step is checked in screw algebra (fact/fact_core.py, stage3/topopt/fact_ball.py); the part is then built in CadQuery (stage3/topopt/fact_ball_cad.py) and exported as STL and STEP.",
  [("motion", f"the six leg lengths are constant over the day (both rings are defined from the head pose), so each ball turns only with the loop's corrections: the production runs used {_fb['dl_run_mm']:.0f} mm at most ({_fb['theta_run_deg']:.2f} deg at the calyx end); the loop's clip is the joint's design variable and is +-{_fb['clip_mm']:.0f} mm ({_fb['theta_deg']:.2f} deg for one leg at full stroke)"),
   ...
   ("what the solid changed", f"the section drawing sized the blades freely and chose 120 mm wide at 36 mm from C. Three legs on a cone share an arc, so a blade whose width runs circumferentially can only be {2*3.1416*36*0.574/3:.0f} mm there; and the hub that carries the 85 mm strut tube pushes the blades out along the leg, where the (1 + 6 s/L) amplification eats the stress budget. Sizing inside the CAD, per blade and against the geometry, is the only way the numbers and the part agree"),
   ("the tripod, as built", f"cone {_cT['alpha_deg']:.0f} deg (searched: the smallest joint the strut's diameter allows); blades {1e3*_pT['t']:.1f} mm thick, {1e3*_pT['w_B']:.0f} mm wide circumferentially and {1e3*_pT['w_A']:.0f} mm meridionally, {1e3*_pT['L']:.0f} mm free, at {1e3*_pT['s_in']:.0f} and {1e3*_pT['s_out']:.0f} mm from C: {_pT['sig']/1e6:.0f} MPa at the clip, Euler {_pT['Pcr']/1e3:.0f} kN, joint {_pT['k_j']/1e6:.0f} MN/m = {_pT['k_j']/(200e9*1.33e-3/1.446)/1e0:.1f} x the strut's own EA/L. {_cT['envelope_mm']['d']:.0f} x {_cT['envelope_mm']['h']:.0f} mm, {_cT['mass_kg']:.2f} kg of Ti"),
   ("what building it argued", f"the same three rotations come from ONE turned waist {2e3*_pN['r']:.1f} mm across and {1e3*_pN['h']:.1f} mm long: {_pN['sig']/1e6:.0f} MPa at the same rotation, the same {_pN['k_j']/1e6:.0f} MN/m, {_cN['envelope_mm']['d']:.0f} x {_cN['envelope_mm']['h']:.0f} mm and {_cN['mass_kg']:.2f} kg, at the price of a {_pN['k_th']*_pN['theta']:.0f} N m restoring moment ({_pN['k_th']*_pN['theta']/1.446:.0f} N at the strut, against loads of kN) and {_pN['sig_c']/1e6:.0f} MPa of compression at the 40 m/s gust. For this duty the waist is the better part; the tripod is the chapter's construction and the one that makes the constraint topology visible"),
   ("to print", "flange down, no support: every blade stands within 35 deg of the vertical and the waist's cones are 45 deg. Four files in stage3/topopt/out: the two machine parts in Ti (STEP for the wire-EDM or the lathe) and two demonstrators in PLA sized so the printed part reaches its rotation at the same fraction of its own yield, the tripod at +-1.5 deg over 213 x 120 mm and the waist at +-4 deg over 109 x 139 mm. Three stops on the tripod touch at 1.5 x the design rotation so a model cannot be snapped; a groove marks the C plane on both")],
  "Twelve joints, no bearings, no backlash, no lubrication. An opus audit (two verifiers per finding) corrected the stress rule, the rotation and the load citations of the first version; building the solid then refuted its blade widths and its cone angle. stage3/topopt/fact_ball.py, fact_ball_cad.py, out/audit_findings.json."))

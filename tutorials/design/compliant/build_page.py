...
BEHIND.append(plate3d(46, "Everything behind the dish: Cassegrain head on its style, hollow flexure neck, one stalk, the beam down inside (equinox noon)", CD + "cd_equinox_noon.json",
  ...
  [("optics", f"secondary r {_O['r_sec']:.2f} m at {_O['D_SEC']} m (magnification {_O['m']:.1f}, f_eff {_O['f_eff']:.0f} m); hole r {_O['r_hole']:.2f} m; image r {_O['r_img']:.2f} m 1.0 m behind the vertex; M3 r {_O['r_m3']:.2f} m, M4 r {_O['r_m4']:.2f} m, both at 45 deg at every elevation"),
   ("shading", "raster along the sun line: secondary 2.3 %, rim toroid 0.1 % of the net annulus: 2.45 %; of the full aperture with the hole and the sag-correct secondary 3.1-3.3 % (audit SH-2, SH-4, OPT-11). The tube machine on the same denominator: 14.6 % at el 83, 15.5 % at el 59, 16.0 % at el 37 (the slot adds nothing: it lies under the tube's shadow, audit SH-3, OPT-04)"),
   ("light", "4 reflections above ground instead of 2. AUDITED: the 1.03-1.14 x gain was measured against a double-counted baseline (the slot lies under the tube's shadow); on one denominator the fourth pass blocks 3.1-3.3 % and the tube machine 14.6-16.0 %, break-even reflectivity 0.925, and at 0.88 the fourth pass delivers LESS light; its style and spine also intercept the converging beam (audit SH-1, OPT-02, OPT-05)"),
   ...
   ("head", "dish + style + secondary + M3 on the diaphragm fine stage (sheets 38-39) on a short frame; water counterweights 151 kg behind the neck and 267 kg beyond the stalk (the head and its neck counterweight both sit 1.4 m from the stalk), the neck one in the shadow, the 267 kg one 3.0 m from the dish axis and NOT in it (audit R-13); screw jack 1.40-2.11 m from the yoke to a 0.8 m crank 20 deg from -n toward the dish (arm >= 0.39 m): 4 / 31 kN at 9 / 25 m/s"),
   ("size", f"stalk to the neck at env z {_O['z_neck']:.2f} m; lowest rim 0.2 m over the deck at el 12; the head sweeps 5.6 m about the neck by its rim and 10.3 m by its secondary; the roof a circle of 10.7 m about the stalk at el 12 (audit R-02, OPT-06); F2 at env z 6.14 is kept, so the rim strikes the deck if the machine is scaled by more than 1.26 (audit R-03)")],
...
BEHIND.append(plate3d(48, "Hollow cartwheel trunnion: radial blades in planes through the neck axis around the beam's bore", CD + "cd_pivot.json",
  ...
  [("check", "18 lines per stage, rank 5, DOF 1 = rotation about the neck; reciprocal products 0; the axis line empty"), ("range", "AUDITED: the +-25.8 deg and 208 N m/rad used the straddling-strip formulas; for an annular blade clamped at r 0.45 and driven at r 0.33 the curvature is 121 theta/m and the stiffness 2144 N m/rad per blade, 12.9 kN m/rad per stage (audit MS-01, MS-02): the range at 0.25 sigma_y is a few degrees, not 26"),
   ("loads", "AUDITED: the head's weight makes no moment about the vertical (it bends the pivot in the vertical plane, 2.7 kN m on the ground stage); the crosswind alone is 1.05 / 8.1 kN m at 9 / 25 m/s mean; one blade per loaded plane, not two: 2.7 / 8.8 kN, SF 4.2 / 1.3 against 11.4 kN, and the buckling mode checked is the wrong one for this load (audit MS-03, MS-04, MS-05, R-07, R-08); with a gust factor of 3 the pivot fails at 25 m/s")], legend(CD + "cd_pivot.json")))

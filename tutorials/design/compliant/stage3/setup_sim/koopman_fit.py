...
def rollout(model, x0, U):
    xs = [x0]
    for k in range(len(U) - 1): xs.append(model.predict(xs[-1].reshape(1, -1), U[k].reshape(1, -1)).ravel())
    return np.array(xs)
out = []
for name, obs in (("DMDc (identity)", Identity()), ("EDMDc (poly deg 2)", Polynomial(degree=2))):
    ...
    ro = rollout(model, X[n_tr], U[n_tr:]); er = np.sqrt(np.mean((ro - X[n_tr:])**2, axis=0))
    out.append(f"{name}: train {n_tr} steps, test {n - n_tr}; one-step RMS error vertex {np.linalg.norm(e1[:3])*100:.1f} cm, axis {e1[3]*100:.1f} cm, pointing {e1[4]:.2f} deg; "
               f"{(n - n_tr)*dt:.0f} s roll-out RMS vertex {np.linalg.norm(er[:3])*100:.1f} cm, pointing {er[4]:.2f} deg")

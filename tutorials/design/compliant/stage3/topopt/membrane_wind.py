...
ax = fig.add_subplot(1, 3, 1); ax.plot(r, 1e3*dslope, color="#d9480f", label=f"+50 Pa on the differential: f {f0:.2f} -> {f1:.2f} m, {1e3*rms_dslope_50:.1f} mrad rms")
ax.plot(r, 1e3*dslope*atten*58.4/50, color="#0e7490", label=f"the 9 m/s mean through a sealed plenum ({atten*58.4:.1f} Pa reaches the film)")
ax.axhline(0, color="k", lw=0.5); ax.set_xlabel("r (m)"); ax.set_ylabel("change of surface slope (mrad)"); ax.legend(fontsize=7, loc="upper left"); ax.set_title("1-D zoned FvK: what a uniform pressure change does to the slope", fontsize=9)
...
ax.set_xscale("log"); ax.set_yscale("log"); ax.set_xlabel("pressure-loop corner frequency (Hz), plenum on a constant-pressure supply"); ax.set_ylabel("defocus blur at F from the gust residual (cm rms)"); ax.legend(fontsize=8, loc="lower left"); ax.set_title("gust residual: a pressure loop (curves) against a sealed plenum (dotted)", fontsize=9)
ax = fig.add_subplot(1, 3, 3); im = ax.imshow(1e3*np.where(inside, dw, np.nan).T, origin="lower", extent=[-ext, ext, -ext, ext], cmap="RdBu_r"); plt.colorbar(im, ax=ax, label="mm"); ax.set_title(f"the gradient's n = 1 mode at 15 m/s (2-D FvK difference, mm): slope {1e3*fig2d:.1f} mrad rms, mean {1e3*tilt2d:.1f}", fontsize=9)

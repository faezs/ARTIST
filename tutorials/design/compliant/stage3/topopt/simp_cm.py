...
a density filter (the remedy Frecker cites for mesh dependence and one-node hinges), input and output springs, the objective
'maximise the output displacement' (Eq. 7.5) solved by MMA (sec. 7.6). The structure follows Sigmund's 99/88-line codes that
the chapter points to (refs 10, 12).

Validation: the displacement inverter of Fig. 7.8 (TopOpt applet: input at the left middle, output at the right middle, the
two left corners fixed). The flower's calyx is not a rectangle in plane stress; it is a disc behind a membrane without a hole (the production optics are a
beam-down that never passes through the dish) and is modelled as a flat shell in plate_disc.py."""
...
class Simp:
    ...
    def run(self, f, out_dof, vol_frac, iters=150, passive=None, verbose=False, tag="", out_sign=1.0):
        ...
        for it in range(iters):
            ...
            obj = -out_sign*float(u[out_dof]); dobj_phys = self.p*xphys**(self.p - 1)*(self.E0 - self.Emin)*ce         # d(-u_out)/dx_e = lam^T dK/dx u
            ...
            if verbose and (it % 20 == 0 or it == iters - 1): print(f"  [{tag}] it {it:3d} u_out {-obj:+.4f} vol {xphys.mean():.3f} change {ch:.4f}")
            if it > 40 and ch < 0.01: break
        xphys = np.asarray(self.H@x).ravel()/self.Hs
        if passive is not None: xphys = np.where(passive == 1, 1e-3, np.where(passive == 2, 1.0, xphys))
...
def case_inverter(nelx=60, nely=30):
    """Fig. 7.8: the full domain (no symmetry), left corners fixed, input force to the right at the left middle, output to the
    right at the right middle expected negative (the inverter), springs k_in = k_out = 0.1 (the 88-line convention)"""
    ...
    xphys, u, hist = s.run(f, out, 0.3, iters=150, tag="inverter", out_sign=-1.0); return s, xphys, u, dict(u_in=float(u[2*s.node(0, nely//2)]), u_out=float(u[out]))
if __name__ == "__main__":
    rep = {}; fig, ax0 = plt.subplots(1, 1, figsize=(12, 4.5)); axs = [ax0]
    s, xphys, u, info = case_inverter(); rep["inverter"] = info
    axs[0].imshow(1 - s.image(xphys), cmap="gray", vmin=0, vmax=1, origin="lower"); axs[0].set_title(f"Fig. 7.8 by SIMP (60 x 30, p 3, filter 2.4, MMA): the displacement inverter; u_in {info['u_in']:+.2f}, u_out {info['u_out']:+.2f}", fontsize=9); axs[0].set_axis_off()
    fig.suptitle("Chapter 7 by SIMP (Frecker, Eq. 7.3-7.5): the book's displacement inverter, Fig. 7.8", fontsize=11)

...
This copy lives beside the envs so tandoor_flower_env can import it; the design branch keeps the original at
design/compliant/stage3/fact/screw_render.py. Keep the two identical.
...
def _wrenches(members, origin=None):
    ...
    Moments are taken about `origin` - the screw's own point - so the answer does not depend on where the world's
    origin happens to be (about the world origin a joint 6 m from it scored 13 where the same joint at the origin
    scored 35249: the translation part of the scaled twist swamped the rotation)."""
    out = []; o = np.zeros(3) if origin is None else np.asarray(origin, float)
    for m in members:
        if m["kind"] in ("pin", "rigid slide", "slaving link"): continue
        E = m.get("E", STEEL["E"]); v = m["verts"]
        if len(v) < 4: continue
        p = v.mean(axis=0) - o; d = v[3] - v[0]; L = float(np.linalg.norm(d))
        if L < 1e-9: continue
        d = d/L; t, wd = m["t"], m["w"]
...
def evaluate(members, screws, mat=STEEL, free_tol=1e-9, scale=None):
    ...
    for (name, w, q, pitch, travel, load) in screws:
        ...
        want = _twist(w, np.zeros(3), pitch)                  # about the screw's own point: frame-invariant

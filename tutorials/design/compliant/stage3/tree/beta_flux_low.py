...
if __name__ == "__main__":
    ...
    say("THE POWER BELOW BETA 36, on the machine as flower.ini configures it (tri, r_duct 0.55, roof deck, post_offset 0.5)")
    ...
    for day, name in zip(DAYS, ("midwinter (doy 355)", "equinox (doy 80)", "midsummer (doy 172)")):
        ...
        for beta in BETAS:
            ...
            del e, S
        say()
    open(os.path.join(OUT, "beta_flux_low.txt"), "w").write("\n".join(lines) + "\n")
    json.dump(res, open(os.path.join(OUT, "beta_flux_low.json"), "w"), indent=1)

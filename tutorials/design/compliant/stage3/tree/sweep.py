...
lines = [f"the head on the env's retro orbit (beta_dev 0): hub over the deck x {hub[:,0].min():.2f}..{hub[:,0].max():.2f} m (north of the pipe), y +-{np.abs(hub[:,1]).max():.2f} m, z {hub[:,2].min():.2f}..{hub[:,2].max():.2f} m; a box {np.ptp(hub[:,0]):.1f} x {np.ptp(hub[:,1]):.1f} x {np.ptp(hub[:,2]):.1f} m",
         ...
         f"pipe + strip shadow at retro: the env's measured table gives 33 % of the aperture (beta_dev 36 takes it to 1.6 % with the head riding higher and further out)",
         f"wind on the head: drag {q9*A_D*1.3/1e3:.2f} / {q12*A_D*1.3/1e3:.2f} / {q25*A_D*1.3/1e3:.1f} kN at 9 / 12 / 25 m/s (Cd 1.3, rho 1.03); to keep half power the crown must hold the head's centre of curvature to 5 cm = a crown stiffness of {q9*A_D*1.3/0.05/1e3:.0f} kN/m at 9 m/s, {q12*A_D*1.3/0.05/1e3:.0f} kN/m at 12 m/s, before gusts; a 5 m cantilever needs EI {q9*A_D*1.3/0.05*125/3/1e3:.0f} kN m2 for that (one 150 x 5 mm steel tube is 380 kN m2; an inflated hose r 0.25 at 40 kPa wrinkles at 1.0 kN m, i.e. 0.2 kN at 5 m)"]

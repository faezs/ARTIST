...
STATE_W, CMD_W, SUN_W, PRM_W = 3, 2, 3, 8
assert len(INPUTS) == STATE_W + CMD_W + 1 + SUN_W + PRM_W, INPUTS
...
def _kernel_body():
    """the calls of the six functions, from the manifest: the same 17 arguments, each into its slice"""
...
kernel void hashemi_mega(device float* st [[buffer(0)]], device const float* cmd [[buffer(1)]],
                         ...
                         device const int* nB [[buffer(6)]], uint b [[thread_position_in_grid]]) {{
...
extern "C" __global__ void hashemi_mega(float* st, const float* cmd, const float* sun, const float* prm,
                                        const float* dtb, float* out, const int* nB) {{
...
def mega_numpy(state, cmd, dt, sun, params):
    """the row for every agent from the NumPy twin: state (B,3), cmd (B,2), sun (B,3), params (8,) or (B,8)"""
    import hashemi_ccc as H
...
class HashemiMetal:
    ...
    def step(self, state, cmd, dt, sun, params):
        """state (B,3) float32 mps, updated in place; cmd (B,2); sun (B,3); params (8,) or (B,8); returns out (B,N_COLS)"""
        torch = self.torch
        B = state.shape[0]

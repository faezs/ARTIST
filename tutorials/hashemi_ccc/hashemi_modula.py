...
class ModulaMetal:
    ...
    def jvp(self, name, x, dx):
        """x, dx (N, n_in) float32 -> y, dy (N, n_out)"""
        torch, m = self.torch, MODULES[name]
        N = x.shape[0]
        y = torch.empty(N, m["n_out"], dtype=torch.float32, device="mps")
        dy = torch.empty_like(y)
        getattr(self.lib, m["kernel_jvp"])(x.contiguous(), dx.contiguous(), y, dy,
                                          torch.tensor([N], dtype=torch.int32, device="mps"), threads=N)
        return y, dy

    def box(self, name, lo, hi, sc):
        """lo, hi, sc (N, n_in) float32 -> olo, ohi, oL (N, n_out)"""
        torch, m = self.torch, MODULES[name]
        N = lo.shape[0]
        olo = torch.empty(N, m["n_out"], dtype=torch.float32, device="mps")
        ohi = torch.empty_like(olo)
        oL = torch.empty_like(olo)
        getattr(self.lib, m["kernel_box"])(lo.contiguous(), hi.contiguous(), sc.contiguous(), olo, ohi, oL,
                                          torch.tensor([N], dtype=torch.int32, device="mps"), threads=N)
        return olo, ohi, oL
...
def soundness(mm, rng, n_boxes=64, n_probe=32, width=0.05):
    ...
    for name, m in MODULES.items():
        ...
        olo, ohi, oL = mm.box(name, t32(lo), t32(hi), t32(sc))
        L = oL.cpu().numpy().astype(np.float64)
        # probes: points in the box, unit directions in the inf-norm on the real inputs
        ...
        rows.append((name, n_in, n_out, int(finite.sum()), int((L >= HK_INF).sum()), int(viol.sum()),
                     float(np.max(np.where(finite & (L > 0), meas / np.maximum(L, 1e-30), 0.0)))))
    return rows
...
def dish_stage(mm, rng, el, az, dpose, P, design=False, t_dead=1.076):
    ...
    return dict(cap_lo=float(olo[:, 0].mean()), cap_hi=float(ohi[:, 0].mean()), cap=float(y[:, 0].mean()),
                L_bound=float(np.mean(np.minimum(oL[:, 0], HK_INF))), L_inf_frac=float((oL[:, 0] >= HK_INF).mean()),
                L_meas=float(np.mean(np.where(np.isfinite(dy[:, 0]), dy[:, 0], 0.0))),
                L_meas_max=float(np.max(np.where(np.isfinite(dy[:, 0]), dy[:, 0], 0.0))))
...
def megastep_action_sensitivity(mm, rng, B=2048):
    ...
    ca, ct = COL["az_next"], COL["t_next"]
    fin = lambda a: a[np.isfinite(a) & (a < HK_INF)]
    return dict(az_per_omegam_meas=float(np.max(dy_m[:, ca])), t_per_omegad_meas=float(np.max(dy_d[:, ct])),
                az_bound_finite=float(np.mean(oL[:, ca] < HK_INF)), t_bound_finite=float(np.mean(oL[:, ct] < HK_INF)),
                az_bound_med=float(np.median(fin(oL[:, ca]))) if fin(oL[:, ca]).size else float("inf"),
                t_bound_med=float(np.median(fin(oL[:, ct]))) if fin(oL[:, ct]).size else float("inf"))
...
def gates(mm, rng, B=4096, tau=0.01):
    ...
    for name, sig in (("SunReachable", None), ("sunReachableS", 1.0 / (4 * tau))):
        ...
        _, _, oL = mm.box(name, t32(c - w), t32(c + w), t32(sc))
        oL = oL.cpu().numpy().astype(np.float64)[:, 0]
...
def sheaf(mm, rng, lat, day, P, dpose):
    ...
    for hour in range(8, 17):
        ...
        for sgn in [(a, b, c, d) for a in (-1, 1) for b in (-1, 1) for c in (-1, 1) for d in (-1, 1)]:
            ...
            olo, ohi, oL = mm.box("dishPower", t32(c - w), t32(c + w), t32(sc))
            fine_L.append(np.minimum(oL.cpu().numpy()[:, 0], HK_INF).mean()); fine_lo.append(olo.cpu().numpy()[:, 0].mean()); fine_hi.append(ohi.cpu().numpy()[:, 0].mean())
...
def main():
    ...
    print(f"   dishPower at noon, pose/sun within {args.dpose} rad: capture {d['cap']:.3f} in [{d['cap_lo']:.3f}, {d['cap_hi']:.3f}];"
          f" L bound {d['L_bound']:.3g} (inf on {100 * d['L_inf_frac']:.0f} % of rays), measured mean {d['L_meas']:.3g} max {d['L_meas_max']:.3g} per rad")
    dd = dish_stage(mm, rng, el, az, args.dpose, args.rays, design=True)
    print(f"   dishPower with R f a k learnable (design mode, mass 4): L bound {dd['L_bound']:.3g}, measured max {dd['L_meas_max']:.3g}")
    ms = megastep_action_sensitivity(mm, rng)
    print(f"   megaStep: az per unit omega_m measured {ms['az_per_omegam_meas']:.3g} rad (bound finite on {100 * ms['az_bound_finite']:.0f} %, median {ms['az_bound_med']:.3g});"
          f" t per unit omega_d measured {ms['t_per_omegad_meas']:.3g} (bound finite on {100 * ms['t_bound_finite']:.0f} %, median {ms['t_bound_med']:.3g})")
    g = gates(mm, rng)
    for name, r in g.items():
        thm = f", the theorem's slope {r['theorem_slope']:.1f}" if r["theorem_slope"] else ""
        print(f"   {name:<14}: jumps on {100 * r['jump_frac']:.0f} % of boxes, L max (finite) {r['L_max_finite']:.3g}, measured max {r['meas_max']:.3g}{thm}")
    ...
    print("   hour  sun el   capture   [lo, hi] coarse   [lo, hi] refined   L coarse  L refined(max)  glued")
    glue_ok = True
    for r in sh:
    ...
    s_env = ms["az_per_omegam_meas"] * max(d["L_meas_max"], 1e-9)     # capture per unit action: pose per command x capture per rad
    policy, loop = closed_loop(s_env=s_env)
    print(f"   env sensitivity (capture per unit azimuth command) = {ms['az_per_omegam_meas']:.3g} rad/cmd x {d['L_meas_max']:.3g} /rad = {s_env:.3g}")

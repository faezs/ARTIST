...
partial def leanExpr (g : Graph) (real : Bool) (i : Nat) : String :=
  ...
  | .un op a =>
    ...
    | "sqrt" => if real then s!"(Real.sqrt {r a})" else s!"(Float.sqrt (max {r a} 0))"
    ...
    | "arccos" => if real then s!"(Real.arccos {r a})" else s!"(Float.acos (min (max {r a} (-1)) 1))"
    | "arcsin" => if real then s!"(Real.arcsin {r a})" else s!"(Float.asin (min (max {r a} (-1)) 1))"
...
def npRhs (f : Fun) (L : Layers) (i : Nat) : Option String := Id.run do
  ...
  | .un op a =>
    let av := t a
    some (match op with
      | "neg" => s!"(-{av})" | "not" => s!"np.logical_not({av})" | "sqrt" => s!"np.sqrt(np.maximum({av}, 0.0))"
      | "sin" => s!"np.sin({av})" | "cos" => s!"np.cos({av})" | "tan" => s!"np.tan({av})"
      | "arctan" => s!"np.arctan({av})" | "arccos" => s!"np.arccos(np.clip({av}, -1.0, 1.0))" | "arcsin" => s!"np.arcsin(np.clip({av}, -1.0, 1.0))"

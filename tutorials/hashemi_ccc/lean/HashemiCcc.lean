...
def run : MetaM Unit := do
  ...
  let mods := defs.filter fun f => f.inputs.size > 0
  ...
  for f in mods do
    ...
    mk := mk.push s!"kernel void mk_{short}_jvp(device const float* x [[buffer(0)]], device const float* dx [[buffer(1)]], device float* y [[buffer(2)]], device float* dy [[buffer(3)]], device const int* n [[buffer(4)]], uint i [[thread_position_in_grid]]) \{"
    ...
    mk := mk.push s!"  {c}_jvp({args}, tdx, ty, tdy);"
    mk := mk.push s!"  for (int k = 0; k < {nout}; ++k) \{ y[i*{nout}+k] = ty[k]; dy[i*{nout}+k] = tdy[k]; }"
    mk := mk.push "}"
    mk := mk.push s!"kernel void mk_{short}_box(device const float* lo [[buffer(0)]], device const float* hi [[buffer(1)]], device const float* sc [[buffer(2)]], device float* olo [[buffer(3)]], device float* ohi [[buffer(4)]], device float* oL [[buffer(5)]], device const int* n [[buffer(6)]], uint i [[thread_position_in_grid]]) \{"
    mk := mk.push s!"  if ((int)i >= n[0]) return;"
    ...
    mk := mk.push s!"  {c}_box(tlo, thi, tsc, rlo, rhi, rL);"
